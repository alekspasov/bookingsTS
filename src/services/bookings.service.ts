...
export const getAllAirports = async() : Promise<AirportType[]> => {
    ...
    export const getAirportById = async(id : number) : Promise<AirportType> => {
        ...
        export const getAllBookings = async(index: number=0) : Promise<BookingType[]> =>  {
            try{
                const bookings = await fetch(`${BASE_URL}/bookings?pageIndex=${index}&pageSize=5&authToken=${API_KEY}`);
            ...
            export const createNewBooking = async(booking: BookingType) : Promise<BookingType> => {
          
            try{ 
               const newBooking = await fetch(`${BASE_URL}/bookings/create?authToken=${API_KEY}`, 
               {
                method: 'POST',
            ...
            export const deleteBooking = async(bookingId : number) => {
                try{
                   const response = await fetch(`${BASE_URL}/bookings/delete/${bookingId}?authToken=${API_KEY}`,{method:'DELETE'})
